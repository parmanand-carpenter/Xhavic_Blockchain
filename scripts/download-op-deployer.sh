...
download_op_deployer() {
    ...
    # Get all releases and find the latest stable one with op-deployer assets
    # Exclude release candidates (-rc), beta, and alpha versions to get stable versions only
    local latest_deployer_release
    latest_deployer_release=$(curl -s "$releases_url?per_page=50" | jq -r '.[] | select(.tag_name | startswith("op-deployer/")) | select(.tag_name | contains("-rc") | not) | select(.tag_name | contains("-beta") | not) | select(.tag_name | contains("-alpha") | not) | .tag_name' | sort -V | tail -1)
